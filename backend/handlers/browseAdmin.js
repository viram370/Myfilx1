...
const TITLES_PAGE_SIZE = 8;
const TITLE_CACHE_TTL_MS = 20_000;
const PENDING_TTL_MS = 2 * 60_000;
const MAX_EPISODE_BUTTONS = 96; // Telegram inline-keyboard/message-size safety cap
...
async function renderTitleList(chatId, category, page, messageId) {
  ...
  const totalPages = Math.max(1, Math.ceil(titles.length / TITLES_PAGE_SIZE));
  const clamped = Math.min(Math.max(page, 0), totalPages - 1);
  const slice = titles.slice(clamped * TITLES_PAGE_SIZE, clamped * TITLES_PAGE_SIZE + TITLES_PAGE_SIZE);
  const code = CATEGORY_CODE[category];
  ...
  const text = titles.length
    ? `📂 <b>${label}</b> — ${titles.length} title${titles.length === 1 ? '' : 's'}\n\nSelect one to manage:`
    : `📂 <b>${label}</b>\n\nNothing here yet.`;

  const rows = slice.map((t) => [{ text: t.title.slice(0, 60), callback_data: `bx:t:${titles.indexOf(t)}` }]);
  const navRow = [];
  if (clamped > 0) navRow.push({ text: '⬅ Prev', callback_data: `bx:cat:${code}:${clamped - 1}` });
  if (titles.length) navRow.push({ text: `${clamped + 1}/${totalPages}`, callback_data: `bx:cat:${code}:${clamped}` });
  if (clamped < totalPages - 1) navRow.push({ text: 'Next ➡', callback_data: `bx:cat:${code}:${clamped + 1}` });
...
async function renderTitleScreen(chatId, titleIdx, messageId) {
  const state = browseState.get(chatId);
  if (!state) return;
  ...
  const rows = seasons.map((s) => [{ text: `Season ${s}`, callback_data: `bx:s:${s}` }]);
...
async function renderSeasonScreen(chatId, season, messageId) {
  const state = browseState.get(chatId);
  if (!state) return;
  ...
  const capped = episodes.slice(0, MAX_EPISODE_BUTTONS);
  const rows = [];
  for (let i = 0; i < capped.length; i += 4) {
    rows.push(capped.slice(i, i + 4).map((e) => ({ text: `Episode ${e}`, callback_data: `bx:e:${e}` })));
  }
  if (episodes.length > MAX_EPISODE_BUTTONS) {
    rows.push([{ text: `…and ${episodes.length - MAX_EPISODE_BUTTONS} more — use /find episode <N> <docId>`, callback_data: 'bx:noop' }]);
  }
...
async function handleCallback(query) {
  ...
  if (data === 'bx:back:title') { await renderTitleScreen(chatId, state.titleIdx, messageId); await ack(); return; }
  if (data.startsWith('bx:e:')) { await renderEpisodeScreen(chatId, parseInt(data.slice(5), 10), messageId); await ack(); return; }
  if (data === 'bx:back:season') { await renderSeasonScreen(chatId, state.season, messageId); await ack(); return; }
