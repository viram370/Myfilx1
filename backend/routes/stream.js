...
router.get('/:videoId', requireAuth, async (req, res) => {
  try {
    ...
    const fileRes = await axios.get(`https://api.telegram.org/bot${BOT_TOKEN}/getFile`, {
      params: { file_id: video.telegram_file_id },
    });
    if (!fileRes.data.ok) throw new Error('Failed to resolve file from Telegram');

    const streamUrl = `https://api.telegram.org/file/bot${BOT_TOKEN}/${fileRes.data.result.file_path}`;
